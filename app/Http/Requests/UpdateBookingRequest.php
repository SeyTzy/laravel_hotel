...
class UpdateBookingRequest extends FormRequest
{
    ...
    {
        return [
            'booking_code' => ['nullable', 'string', 'max:30', Rule::unique('bookings', 'booking_code')->ignore($this->route('booking'))],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $bookingId = $this->route('booking') instanceof \App\Models\Booking
                ? $this->route('booking')->id
                : $this->route('booking');

            if (! empty($data['room_id']) && ! empty($data['check_in']) && ! empty($data['check_out'])
                && ! Booking::isRoomAvailable($data['room_id'], $data['check_in'], $data['check_out'], $bookingId)) {
